...
export class Window {
  ...
  createWindows(options: object) {
    ...
    for (let i in this.group) {
      if (
        ...
        console.log("the window has existed.");
        this.getWindow(Number(i)).focus();
        return;
      }
    }
    // 创建electron窗口的配置参数
    let opt = this.winOpts([args.width || 800, args.height || 590]);
    // console.log("创建窗口的配置参数", opt);
    console.log("the config options to create a new window: ", opt);
    ...
    console.log("this window's id: " + win, JSON.stringify(win));
    this.group[win.id] = {
      route: args.route,
    ...
    if (args.isMainWin) {
      if (this.main) {
        console.log("the main window is existed.");
    ...
    console.log("the new window's address: ", winURL);
    win.loadURL(winURL);
  ...
  createTray() {
    ...
    this.tray.setToolTip("Little Pig Classroom");
  }
